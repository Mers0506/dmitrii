/*Реализовать класс «Поле для игры в крестики - нолики на доске 3х3».
Требуемые методы : конструктор, деструктор, 
установка крестика / нолика в заданную клетку,
удаление крестика / нолика из заданной клетки,
определение победителя в данной позиции(для выигрыша необходимо составить горизонтальную,
вертикальную или диагональную линию из 3  одинаковых знаков), определение лучшего хода в данной позиции для заданного игрока.
...
int main(void)
{
  ...
  while (!field.isOver())
  {
    ...
    if (type == 1){
      ...
      cout << "Пожалуйста, введите ход 1-3 (номер колонки, пробел, номер строки)" << endl;
